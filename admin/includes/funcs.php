<?php
	...
	/* function usuarioExiste($usuario)
	{
		...
		$stmt = $mysqli->prepare("SELECT id FROM tbladmin WHERE AdminUserName = ? LIMIT 1");
	...
	/* function emailExiste($email)
	{
		...
		$stmt = $mysqli->prepare("SELECT id FROM tbladmin WHERE AdminEmailId = ? LIMIT 1");
	...
	function login($usuario, $password)
	{
		...
		$stmt = $con->prepare("SELECT id, idUserType, password FROM tblusers WHERE userName = ? || email = ? LIMIT 1");
		...
		if($rows > 0) {
			...
			if(isActivo($usuario)){
				...
				if($validaPassw){
					$_SESSION['id'] = $id;
					$_SESSION['Type_Id'] = $id_tipo;
					
					header("location: https://www.cruzrojainstituto.edu.ec/educacioncontinua/admin/dashboard.php");
					} else {
			...
			function alerta(){
			 swal({
			   text: "El nombre de usuario o correo electrónico ingresado no existe",
			   type: "warning",
			 });
			}
			alerta();                   
	...
	function isActivo($usuario)
	{
		...
		$stmt = $con->prepare("SELECT active FROM tblusers WHERE userName = ? || email = ? LIMIT 1");
		$stmt->bind_param('ss', $usuario, $usuario);
		$stmt->execute();
		$stmt->bind_result($activacion);
		$stmt->fetch();
	...
	function getValor($campo, $campoWhere, $valor)
	{
		...
		$stmt = $mysqli->prepare("SELECT $campo FROM tbladmin WHERE $campoWhere = ? LIMIT 1");
	...
	function cambiaPassword($password, $user_id){
		...
		$stmt = $con->prepare("UPDATE tbladmin SET AdminPassword = ? WHERE id = ?");
